...
_, _ = db.Exec(`DROP VIEW IF EXISTS selected_rows3`)
_, _ = db.Exec(`DROP TABLE IF EXISTS selected_rows3`)

	// Create view for partitioned selection
	_, err := db.Exec(`
		CREATE TABLE selected_rows3 AS
		WITH quartiles AS (
			SELECT *, NTILE(4) OVER (ORDER BY id) AS quartile
			FROM campaign_master_1
		)
		SELECT id, column1 FROM quartiles
		WHERE quartile = 3 AND column3 = 1  LIMIT ?`, batchSize)
	if err != nil {
		return nil, nil, fmt.Errorf("TABLE creation failed: %w", err)
	}

	// Select from the view
	rows, err := db.Query("SELECT id, column1 FROM selected_rows3")
	if err != nil {
