...
import { ExternalLink, Phone, Reply, Lock, MoreVertical, ArrowLeft, ShoppingBag, Copy } from 'lucide-react';
...
const Preview: React.FC<PreviewProps> = ({
    ...
    const renderButtons = (btns?: unknown[], compact = false) => {
        if (!btns) return null;

        return (
            <div className={`flex flex-col gap-0.5 mt-2 ${compact ? 'px-2 pb-2' : ''}`}>
                {btns.map((btn: unknown, idx: number) => {
                    const button = btn as { type?: string; text?: string; example?: string };
                    let Icon = Reply;
                    let label = button.text;
                    let color = "text-[#00a884]";
                    const bg = "bg-white";

                    if (button.type === 'URL') Icon = ExternalLink;
                    if (button.type === 'PHONE_NUMBER') Icon = Phone;
                    if (button.type === 'OTP') {
                        label = button.text || 'Copy Code';
                        color = "text-gray-800";
                    }
                    if (button.type === 'COPY_CODE') {
                        Icon = Copy;
                        label = "Copy Code"; 
                        label = button.example ? `Copy Code (${button.example})` : 'Copy Code';
                        color = "text-gray-800";
                    }
                    if (button.type === 'CATALOG' || button.type === 'MPM') {
                        Icon = ShoppingBag;
                        color = "text-gray-800";
                    }
    ...
    return (
        <div className="sticky top-6">
             <div className="relative mx-auto border-gray-800 bg-gray-800 border-[14px] rounded-[2.5rem] h-[690px] w-[340px] shadow-xl">
                    ...
                    <div className="flex-1 p-3 overflow-y-auto custom-scrollbar bg-[url('https://user-images.githubusercontent.com/15075759/28719144-86dc0f70-73b1-11e7-911d-60d70fcded21.png')] bg-repeat opacity-90">
                        ...
                        {carouselComponent && carouselComponent.cards && (
                             <div className="flex overflow-x-auto gap-2 pb-2 -ml-3 px-3 w-[calc(100%+24px)] custom-scrollbar">
                                {carouselComponent.cards.map((card: ComponentJson, idx: number) => {
                                    const cardHeader = card.components?.find((c: ComponentJson) => c.type === 'HEADER');
                                    const cardBody = card.components?.find((c: ComponentJson) => c.type === 'BODY');
                                    const cardButtons = card.components?.find((c: ComponentJson) => c.type === 'BUTTONS');

                                    let cardFile: HeaderFile | undefined = cardHeader?.format === 'IMAGE' ? { url: "https://picsum.photos/200/120", type: 'image' } : undefined;
