...
export default function SendMessageUI({ message, fileType, file, setMessage, setFile, setFileType, onMessageSend, onTemplateMessageSend, disabled = false }: SendMessageUIProps) {
        ...
        <>
            ...
            <form className="bg-rich-text-panel-background px-4 py-2 flex flex-row gap-4 w-30" onSubmit={async (event) => {
                ...
                <input 
                    ...
                    disabled={disabled || messageSendInProgress}
                />
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <Button disabled={typeof file !== 'undefined' || disabled || messageSendInProgress} variant="outline"><Paperclip className="h-4 w-4" /></Button>
                    </DropdownMenuTrigger>
                ...
                <Button type="submit" className="w-32" disabled={disabled || messageSendInProgress}>
                    {messageSendInProgress ? <TWLoader className="w-4 h-4" /> : 'Send'}
                </Button>
            </form>
        </>
    )
