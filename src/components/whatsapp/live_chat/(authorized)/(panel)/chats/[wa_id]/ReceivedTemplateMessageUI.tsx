...
export default function ReceivedTemplateMessageUI(props: { message: TemplateMessage }) {
    // Safety check: if template or components are undefined, show a fallback message
    if (!props.message.template || !props.message.template.components || !Array.isArray(props.message.template.components)) {
        return (
            <div className="max-w-sm p-2 bg-gray-100 rounded">
                <div className="text-sm text-gray-600">Template message</div>
                {props.message.template?.name && (
                    <div className="text-xs text-gray-500 mt-1">Template: {props.message.template.name}</div>
                )}
            </div>
        )
    }
