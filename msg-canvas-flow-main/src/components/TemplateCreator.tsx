...
import Preview from './whatsapp/preview';
...
const TemplateCreator = ({ isSidebarHovered,initialTemplateJson }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            console.log("FINAL JSON TO SUBMIT:", JSON.stringify(templateJson, null, 2));
            ...
            if (!response.ok) {
                ...
                alert(`Submission failed: ${response.status} ${response.statusText}` + (text ? ` - ${text}` : ''));
            } else {
            ...
            console.error('Network or unexpected error while submitting template:', err);
            alert('Failed to submit template. Check the console for details and ensure the backend is running at http://localhost:8080.');
        } finally {
            setLoading(false);
        }
    };
